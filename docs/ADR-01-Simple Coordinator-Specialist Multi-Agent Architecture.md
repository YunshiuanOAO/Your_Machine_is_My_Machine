...
- `scan/` is created only when `recon_agent` runs the package scanner flow (`RustScan -> Nmap -> Dirsearch/WhatWeb`). If the operator supplies artifacts such as `--rustscan-file`, the report references those external paths in `recon_report.json` instead of copying them into `scan/`.
